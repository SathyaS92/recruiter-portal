...
@Component({
  selector: 'app-processed-jobs',
  templateUrl: './processed-jobs.component.html',
  styleUrls: ['./processed-jobs.component.scss']
})
export class ProcessedJobsComponent implements OnInit {
  ...
  private processedJobsService:JobsService;
  ...
  constructor(private http: HttpClient, _processedJobsService:JobsService, private router:Router, private modalService: NgbModal,private dataTransferService:DataTransferService) { 

    this.processedJobsService = _processedJobsService;    
    ...
    this.columnDefs = [
      ...
      { headerName: "Customer", minWidth:165,field: 'customer', tooltipField: 'customer' },
      { headerName: "Job Description", minWidth:197,field: 'jobDescription', onCellClicked: this.ViewDescriptionOnModel.bind(this),cellRendererFramework: JobDescriptionRendererComponent },
      { headerName: "Job Location", minWidth:161,field: 'jobLocation', tooltipField: 'jobLocation' },
      { headerName: "Total Openings", minWidth:151,field: 'numOfOpenings',cellRendererFramework: TextHighlightRenderer },
      { headerName: "Notified Candidates",  minWidth:186, cellRendererFramework: NotificationComponent },
      { headerName: "Yet to be Notified Candidates", minWidth:241, cellRendererFramework: NotificationComponent },
      { headerName: "Interested Candidates", minWidth:199, cellRendererFramework: NotificationComponent  },
      { headerName: "Not responded Candidates", minWidth:239, cellRendererFramework: NotificationComponent  },
      { headerName: "Job Added Date", minWidth:166,field: 'jobAddedDate' },
  ...
  ngAfterViewInit(){
    ...
    this.dataTransferService.currentRefresh.pipe(takeWhile(() => this.alive)).subscribe((data:string) => {
      if(data == "processed"){   
      this.processedJobsService.getProcessedJobs(this.userID,this.clientIDS).subscribe(data => {            
